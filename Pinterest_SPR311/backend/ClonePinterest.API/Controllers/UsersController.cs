...
public class UsersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetUserProfile(int id)
    {
        try
        {
            ...
            // block information (only relevant if current user is authenticated)
            if (currentUserId.HasValue)
            {
                profile.IsBlockedByMe = await _context.UserBlocks
                    .AnyAsync(b => b.BlockerId == currentUserId && b.BlockedId == id);
                profile.HasBlockedMe = await _context.UserBlocks
